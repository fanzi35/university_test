...
\begin{enumerate}
    ...
    \begin{equation}
        \sigma_t=\frac{P\cdot e}{W_b}=\frac{T\cdot e}{KdW_b}
    \end{equation}
...
适用性：
\begin{enumerate}
    \item 可用于研究偏心距对最大预紧力矩的影响:从模型结果可以看出两者大致是成反比的，符合材料力学中偏心拉伸构件的基本受力规律。
...
\begin{enumerate}
    ...
    \right\}
    \end{equation}
    \item 锚固系统约束：这一约束主要来自2锚固剂与围岩的极限粘结力$F_{bond}$，而$F_{bond}$的计算过程中所用的钻孔直径，有效锚固长度和围岩与锚固剂界面的剪切强度与偏心距e无明显关联，无需修正。
    \item 围岩压陷极限：偏心距e的存在使得托盘不仅受到轴向压力P还会受到由偏心距e引起的附加弯矩M=Pe，假设托盘为边长为b的方形，在托盘与围岩保持完全接触、接触压力沿偏心方向线性变化的条件下，最大接触压强可表示为\cite{ivan2010eccentric}
    \begin{equation}
        q_i
    =
    \frac{P}{A}
    \left(
        ...
        \frac{y_i e_y}{i_x^2}
    \right),
    \label{eq:eccentric-pressure-general}
    \end{equation}
...
\begin{enumerate}
    ...
    \begin{equation}
        P\le \frac{10\delta_{max}\cdot \eta\cdot M_Rf\cdot 10^{-2}\cdot b^2}{(1-\nu^2)h_o}
    \label{q4-lim3}
    \end{equation}
...
\begin{aligned}
    ...
    & P_3=\frac{10\delta_{max}\cdot \eta\cdot M_Rf\cdot 10^{-3}\cdot b^2}{(1-\nu^2)h_o},\\
\end{aligned}
\end{equation*}

则有:
\begin{equation}
    T_{opt}=Kd\cdot \min(P_1,P_2,P_3)
\label{q4-Topt}
\end{equation}
...
\begin{split}
    R_{23}(f) &= \frac{P_2}{P_3} 
         = \frac{\Pi\cdot D_{hole}\cdot L_{bond}\cdot (0.25\eta M_Rf\cdot 10^{-2}+1.0)\cdot 10^{-3}}{\frac{10\delta_{max}\cdot \eta\cdot M_Rf\cdot 10^{-2}\cdot b^2}{(1-\nu^2)h_o}} \\
...
对于有效性与可靠性的验证，我们选择使用附件表4中提供的工况数据进行数值验证。
\begin{enumerate}
    \item 对于有效性验证，我们采用满足上界的方法，即依据模型，根据式~\ref{q4-Topt}，计算使支护效果较优而不触发任何失效约束的预紧力矩$T_{opt}$，同时也计算原约束“锚固强度极限”、“锚固系统极限”、“围岩压陷极限”，并通过比较验证模型所得$T_{opt}$仍可满足原约束，以此证明模型的有效性。
\end{enumerate}
...
Mathematical Models for Engineering Science, 2010.
